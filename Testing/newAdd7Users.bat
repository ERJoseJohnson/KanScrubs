...
curl -v -d "{username:JohnDoe@mymail.sutd.edu.sg,password:IAmJohnDoes1!,queryType:General}" -H 'Content-Type: application/json' http://localhost:3001/login/?username=JohnDoe@mymail.sutd.edu.sg > user1.txt 2> user1err.txt
curl -v -d "{username:JaneDoe@mymail.sutd.edu.sg,password:IAmJaneDoes1!,queryType:Mortgage}" -H 'Content-Type: application/json' http://localhost:3001/login/?username=JaneDoe@mymail.sutd.edu.sg > user2.txt 2> user2err.txt
curl -v -d "{username:FrankDoe@mymail.sutd.edu.sg,password:IAmFrankDoes1!,queryType:Insurance}" -H 'Content-Type: application/json' http://localhost:3001/login/?username=FrankDoe@mymail.sutd.edu.sg > user3.txt 2> user3err.txt
curl -v -d "{username:JamesDoe@mymail.sutd.edu.sg,password:IAmJamesDoes1!,queryType:Card}" -H 'Content-Type: application/json' http://localhost:3001/login/?username=JamesDoe@mymail.sutd.edu.sg > user4.txt 2> user4err.txt
